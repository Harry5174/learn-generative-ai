...
from ..assistant.assistant import *  # Import the AssistantManager class from assistant.py

import asyncio
from typing import Annotated, Optional

from ..assistant.models.pydantic_models import *
...
# OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')  # Use getenv to avoid KeyError if the variable is not set
# database_url: Optional[str | URL] = os.getenv("DATABASE_URL")  # insert your database URL here

database_url = "postgresql://Harry5174:REDACTED_DB_PASSWORD@ep-shrill-moon-a5xaw1l1-pooler.us-east-2.aws.neon.tech/poetry-assistant?sslmode=require"
...
# Create a new instance of AssistantManager
assistant_manager = AssistantManager(api_key=OPENAI_API_KEY,engine=engine)

app = FastAPI()

@app.post("/internal-action")
async def internal_action(action_name: str, arguments: Optional[dict] = None):
    response = await make_internal_api_call(action_name, arguments)
    return JSONResponse(content=response)

# function for creating the QueryLog and Location tables in the database if not created
...
async def start_conversation():
    thread_info = assistant_manager.start_conversation()
    return thread_info

#  functions which will communicate with the database incorporated within FastAPI endpoints
@app.get("/persons/")
...
async def chat(request: ChatRequest):
    ...
    # request thread message upon receiving thread_id
    message = client.beta.threads.messages.create(thread_id=thread_id, role="user", content=user_input)
    dict(message)

    #run the request to assistant
    run: Run = client.beta.threads.runs.create(thread_id=thread_id, assistant_id=assistant_manager.assistant_id)
    dict(run)
